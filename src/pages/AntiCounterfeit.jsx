...
export default function AntiCounterfeit() {
  ...
  const handleSubmit = (e) => {
    ...
    // Example mock validation (replace with API call if needed)
    if (serialNumber.trim() && authCode.trim()) {
      ...
      setResult({
        status: "error",
        message: "⚠️ Invalid or incomplete data. Please check your inputs.",
      });
    }
  };
        ...
        <div className="mt-10 text-left space-y-4 text-gray-700">
          ...
          <div className="bg-[#fff7f5] border-l-4 border-red-500 p-4 rounded">
            <div className="flex items-start gap-2">
              ...
              <div>
                <h3 className="font-semibold text-gray-800 mb-1">Warning:</h3>
                <p className="text-sm leading-relaxed">
                  We strongly discourage purchasing loose ampoules, trays, or
                  blisters without cartons. Genuine NovaTech products are always
                  supplied in tamper-proof cartons with intact silver scratch
                  fields, except Oral Strips which do not include authentication
                  features.
                </p>
              </div>
            </div>
